...
# テーブル作成SQL
create_sql="""CREATE TABLE items(item_id INTEGER PRIMARY KEY AUTOINCREMENT,
item_name STRING UNIQUE NOT NULL,price INTEGER NOT NULL )"""
cur.execute(create_sql)
print('（２）テーブル作成')

# データ登録SQL
insert_sql="""INSERT INTO items(item_name,price) VALUES(?,?)"""
insert_data_list=[
...
price=500
id=1
cur.execute(updata_sql,(price,id))
print('（６）----データ更新：実行----')
conn.commit()
cur.execute(select_one_sql,(id,))
data=cur.fetchone()
